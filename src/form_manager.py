...
class FormManager:
    ...
    def align_group(form_periods):
        role_aligns = pd.DataFrame(np.vstack(form_periods["coords"].values), columns=["x", "y"])
        ...
        for _ in range(3):
            for i, coords in enumerate(form_periods["coords"]):
                ...
                role_aligns.loc[perm + 10 * i, "activity_id"] = form_periods.at[i, "activity_id"]
                role_aligns.loc[perm + 10 * i, "form_period"] = form_periods.at[i, "form_period"]
    ...
    def align(self, group_type="formation"):
        ...
        self.role_records = pd.merge(
            self.role_records[HEADER_ROLE_SUMMARY],
            self.form_periods[["activity_id", "form_period", "formation"]],
        )
        self.role_records = pd.merge(self.role_records, role_aligns).sort_values(
    ...
    def visualize_group(self, group, group_type="formation", paint=True, annotate=True):
        if self.role_records is not None and "aligned_role" in self.role_records.columns:
        ...
        colors = role_records["aligned_role"] if paint else "gray"

        plt.figure(figsize=(7, 5))
        plt.scatter(role_records["x"], role_records["y"], s=150, alpha=0.5, c=colors, cmap="tab10", zorder=0)

        if annotate:
            mean_coords = role_records.groupby("aligned_role")[["x", "y"]].mean()
